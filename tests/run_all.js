...
async function runAllTests() {
  ...
  const totalPassed = storeResults.passed + serviceResults.passed + screenNavResults.passed;
  const totalFailed = storeResults.failed + serviceResults.failed + screenNavResults.failed;
  const totalTests = totalPassed + totalFailed;

  console.log('\n====================================================');
  console.log(' SUMMARY OF TEST SUITE EXECUTION RESULTS');
  console.log('====================================================');
  console.log(`Total Test Suites Executed : 3`);
  ...
  if (totalFailed > 0) {
    ...
    const allErrors = [...storeResults.errors, ...serviceResults.errors, ...screenNavResults.errors];
